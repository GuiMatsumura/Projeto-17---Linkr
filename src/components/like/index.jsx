...
export default function Like({ postId }) {
  ...
  useEffect(() => {
    ...
    request.catch(() => alert(/* "Erro de autenticação!!!" */ postId));
  }, []);
  ...
  function showLiked() {
    if (!clickedLike) {
      if (usernameLikePost.length > 3) {
        return `${usernameLikePost[0].username}, ${
          usernameLikePost[1].username
        } e outras ${usernameLikePost.length - 2} pessoas`;
      } else if (usernameLikePost.length === 3) {
        return `${usernameLikePost[0].username}, ${
          usernameLikePost[1].username
        } e mais ${usernameLikePost.length - 2} pessoa`;
      } else if (usernameLikePost.length === 2) {
        return `${usernameLikePost[0].username} e ${usernameLikePost[1].username}`;
      } else {
        return `${
          usernameLikePost[0] ? usernameLikePost[0].username : "carregando"
        }`;
      }
    } else {
      if (usernameLikePost.length > 3) {
        return `Você, ${usernameLikePost[1].username} e outras ${
          usernameLikePost.length - 2
        } pessoas`;
      } else if (usernameLikePost.length === 3) {
        return `Você, ${usernameLikePost[1].username} e mais ${
          usernameLikePost.length - 2
        } pessoa`;
      } else if (usernameLikePost.length === 2) {
        return `Você, ${
          usernameLikePost[0].username === defaultUsername
            ? usernameLikePost[1].username
            : usernameLikePost[0].username
        }`;
      } else {
        return `Você`;
      }
